...
a1 = intersect(N{1}, a, 'rows','legacy'); % entries must be included 从参数1，N中提取include元素
a2 = setdiff(a, a1, 'rows'); % interchangeable entries 从a中减去include元素
...
for i = 1 : length(nodeList)
    if i == 1
        ...
        Inclu = [N{1}; a2(1:i-1, :)];   % N带入的include元素每次都必然包含
    end
    Exclu = [N{2}; a2(i, :)];           % N带入的exclude元素每次都必然不包含
    tmp = cell(1, 2);
    tmp{1} = Inclu;
    tmp{2} = Exclu;
    nodeList{i} = tmp;
